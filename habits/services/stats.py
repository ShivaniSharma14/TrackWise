...
def get_total_completed_days(habit):
    ...
    if not habit.is_active:
        return {"total_completed_days": 0}
    logs = habit.logs.all()
    total_completed_days = 0
    for log in habit.logs.all():
        if log.value >= target_value:
            total_completed_days += 1
    return {"total_completed_days": total_completed_days}
...
def get_longest_streak(habit):
    ...
    log_by_date = {log.date: log for log in habit.logs.all()}
...
def get_last_7_days_status(habit):
    ...
    for i in range(6, -1, -1):
        ...
        last_7_days_status.append({"date": str(current_day), "completed": completed})
    return {"last_7_days_status": last_7_days_status}
